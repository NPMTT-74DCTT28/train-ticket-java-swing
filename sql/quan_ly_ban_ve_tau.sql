...
CREATE
DEFINER=`root`@`localhost` PROCEDURE `sp_ThongKeDoanhSo` (IN `p_thang` INT, IN `p_nam` INT)
BEGIN
...
(
    20
) NOT NULL COMMENT 'VD: HN, DN, SG',
    `ten_ga` varchar
(
    100
) NOT NULL COMMENT 'VD: Ga Hà Nội',
    `dia_chi` varchar
(
    255
) DEFAULT NULL,
...
(
    50
) NOT NULL COMMENT 'VD: Ngồi mềm điều hòa, Giường nằm',
    `he_so_gia` decimal
(
    3,
    2
...
(
    20
) NOT NULL COMMENT 'VD: SE1, TN1',
    `ten_tau` varchar
(
    100
) NOT NULL COMMENT 'VD: Tàu Thống Nhất SE1',
    PRIMARY KEY
(
    `id`
),
...
(
    20
) NOT NULL COMMENT 'VD: Toa 1, Toa 2',
    `id_tau` int
(
    11
) NOT NULL,
